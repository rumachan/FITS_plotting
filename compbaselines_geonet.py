...
for reg in regions:
  ...
  for id, day in days:
    ...
    for idx, line in enumerate(lines):
      ...
      names = ['dt', 'obs', 'err']
      #first site
      url= 'http://fits.geonet.org.nz/observation?typeID=e&siteID='+siteid1+'&networkID='+netid1+'&days='+day
      df1e = pd.read_csv(url, names=names, skiprows=1, parse_dates={"Datetime" : ['dt']})
      url= 'http://fits.geonet.org.nz/observation?typeID=n&siteID='+siteid1+'&networkID='+netid1+'&days='+day
      df1n = pd.read_csv(url, names=names, skiprows=1, parse_dates={"Datetime" : ['dt']})
      url= 'http://fits.geonet.org.nz/observation?typeID=u&siteID='+siteid1+'&networkID='+netid1+'&days='+day
      df1u = pd.read_csv(url, names=names, skiprows=1, parse_dates={"Datetime" : ['dt']})
      #second site
      url = 'http://fits.geonet.org.nz/observation?typeID=e&siteID='+siteid2+'&networkID='+netid2+'&days='+day
      df2e = pd.read_csv(url, names=names, skiprows=1, parse_dates={"Datetime" : ['dt']})
      url = 'http://fits.geonet.org.nz/observation?typeID=n&siteID='+siteid2+'&networkID='+netid2+'&days='+day
      df2n = pd.read_csv(url, names=names, skiprows=1, parse_dates={"Datetime" : ['dt']})
      url = 'http://fits.geonet.org.nz/observation?typeID=u&siteID='+siteid2+'&networkID='+netid2+'&days='+day
      ...
      #df.drop(['dtn1','dte2','dtn2'], axis=1, inplace=True)  #drop columns
      ...
      if idx == 0: #first plot for region
        ...
        plt.plot(df['Datetime'], df['xyz'], marker='o', color='black',linestyle='None')
        plt.plot(df['Datetime'], df['xyzf'], marker='None', color='red')
        ...
        title = (reg.capitalize()+ ' baselines length change (mm) plotted at: '+ now.strftime("%Y-%m-%d %H:%M"))
        plt.title(title)
      else: 
